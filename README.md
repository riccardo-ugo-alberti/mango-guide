...
The Map page uses `folium` and `streamlit-folium` to show an editorial world map of public reviews that have both `latitude` and `longitude`. It opens on the whole world with a clean CartoDB Positron tile layer and uses score-aware circle markers.

Only public reviews with coordinates appear on the map. Reviews without coordinates remain visible elsewhere in the guide.
...
The app also creates "Open in Google Maps" links for cards, map popups, and mapped review lists. If coordinates exist, the link is generated as:

```text
https://www.google.com/maps/search/?api=1&query={latitude},{longitude}
```

If coordinates are missing but both Address / Place and City exist, the link uses a normal encoded text search query instead. No Google Maps API key is required, no Google Maps API is integrated, and no billing-related dependency is needed.
